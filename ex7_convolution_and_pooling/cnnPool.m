    ...
    for featureNum = 1:numFeatures
        for row = 1:floor(convolvedDim / poolDim)
            for col = 1:floor(convolvedDim / poolDim)
                regionToPool = convolvedFeatures(featureNum, imageNum, ...
                    ((row-1)*poolDim+1):(row*poolDim), ((col-1)*poolDim+1):(col*poolDim));
                pooledFeatures(featureNum, imageNum, row, col) = mean(regionToPool(:));
            end
        end
    end
end
